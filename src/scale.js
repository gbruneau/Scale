...
class Units {
  ...
  getBestUnit(aSizeInMeter) {
    ...
    for (var aUnitId = 0; aUnitId < this.UnitList.length; aUnitId++) {
      if (
        Math.log10(this.UnitList[aUnitId].SizeInMeter) <=
        Math.log10(aSizeInMeter)
      ) {
...
class ScaleObjects {
  ...
  getObjectBySize(aSizeInMeter) {
    ...
    for (i = 0; i < this.ObjectList.length; i++) {
      ...
      if (curObj.SizeInMeter >= minSize && curObj.SizeInMeter <= maxSize) {
        resObject = curObj;
      }
    }
    return resObject;
  ...
  if (ob1Name !== "" && ob2Name !== "") {
    ...
    if (obj1 != null && obj2 != null) {
      ...
      for (var i = 0; i < objList.ObjectList.length; i++) {
        ...
        else if (objectSizeInM <= 1.2e7) sectionCssClass = "scTravel";
        else if (objectSizeInM <= 1e10) sectionCssClass = "scPlanet";
        else if (objectSizeInM <= 3e16) sectionCssClass = "scSolar";
        ...
        else if (sectionCssClass != oldSectionCssClass) {
          curRowHtml =
            "</tbody><tbody class='" + sectionCssClass + "'>" + htmlRowTemplate;
          oldSectionCssClass = sectionCssClass;
        } else curRowHtml = htmlRowTemplate;
        ...
        curRowHtml = curRowHtml.replace(
          "%osm",
          toNormalScientificString(objectSizeInM) + " m",
        );
        curRowHtml = curRowHtml.replace(
          "%osu",
          toNormalScientificString(objectSizeInUnit) + " " + objectUnit,
        );
        ...
        curRowHtml = curRowHtml.replace(
          "%scu",
          toNormalScientificString(scaledSizeInUnit) + " " + scaledUnitSymbol,
        );
